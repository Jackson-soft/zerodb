...
func (fc *FrontendConn) handleStmtPrepare(sql string) error {
	/*if c.schema == nil {
	  ...
	  n := c.proxy.GetHostGroupNode(defaultRule.Nodes[0])
...
func (fc *FrontendConn) writePrepare(s *Stmt) error {
	var err error
	...
	if s.params > 0 {
		...
		total, err = fc.writeEOFBatch(total, fc.status, false)
		if err != nil {
			return err
		}
	}
	...
	total, err = fc.writePacketBatch(total, nil, true)
	total = nil
	if err != nil {
		return err
	}
...
func (fc *FrontendConn) handleStmtExecute(data []byte) error {
	if len(data) < 9 {
	...
	if paramNum > 0 {
		...
		if err := fc.bindStmtArgs(s, nullBitmaps, paramTypes, paramValues); err != nil {
			return err
		}
	}
	...
	case *sqlparser.Select:
		err = fc.handlePrepareSelect(stmt, s.sql, s.args)
	case *sqlparser.Insert:
		err = fc.handlePrepareExec(s.s, s.sql, s.args)
...
func (fc *FrontendConn) handlePrepareSelect(stmt *sqlparser.Select, sql string, args []interface{}) error {
	/*defaultRule := c.schema.rule.DefaultRule
	  if len(defaultRule.Nodes) == 0 {
	      return errors.ErrNoDefaultNode
	  }
	  defaultNode := c.proxy.GetHostGroupNode(defaultRule.Nodes[0])
	  ...
	  if err != nil {
	      golog.Error("FrontendConn", "handlePrepareSelect", err.Error(), c.connectionId)
	      return err
	  }
...
func (fc *FrontendConn) handlePrepareExec(stmt sqlparser.Statement, sql string, args []interface{}) error {
	/*defaultRule := c.schema.rule.DefaultRule
	  if len(defaultRule.Nodes) == 0 {
	      return errors.ErrNoDefaultNode
	  }
	  ...
	  if err != nil {
	      golog.Error("FrontendConn", "handlePrepareExec", err.Error(), c.connectionId)
	      return err
	  }
...
func (fc *FrontendConn) bindStmtArgs(s *Stmt, nullBitmap, paramTypes, paramValues []byte) error {
	args := s.args
...
func (fc *FrontendConn) handleStmtSendLongData(data []byte) error {
	if len(data) < 6 {
		return mysql.ErrMalformPacket
	}
...
func (fc *FrontendConn) handleStmtReset(data []byte) error {
	if len(data) < 4 {
		return mysql.ErrMalformPacket
	}
...
func (fc *FrontendConn) handleStmtClose(data []byte) error {
	if len(data) < 4 {
		return nil
	}
