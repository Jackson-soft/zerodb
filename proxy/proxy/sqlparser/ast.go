...
// 添加了新的语句解析规则后，一定要在token.go中加上关键字分词
...
const (
	AST_DATABASE_CREATE = "database-create"
	AST_TABLE_CREATE    = "table-create"
	AST_INDEX_CREATE    = "index-create"
	AST_ALTER           = "alter"
	AST_DATABASE_DROP   = "database-drop"
	AST_TABLE_DROP      = "table-drop"
	AST_RENAME          = "rename"

	AST_SHOW_TYPE_TABLES        = "tables"
	AST_SHOW_TYPE_DATABASES     = "databases"
	AST_SHOW_TYPE_COBAR_CLUSTER = "cobar_cluster"
)

func (node *DDL) Format(buf *TrackedBuffer) {
	switch node.Action {
	case AST_TABLE_CREATE:
		buf.Fprintf("%s table %s", node.Action, node.NewName)
	case AST_DATABASE_CREATE:
		buf.Fprintf("%s database %s", node.Action, node.NewName)
	case AST_RENAME:
...
func (node *Limit) RewriteLimit() (*Limit, error) {
	...
	if r, ok := node.Rowcount.(NumVal); !ok {
		...
		if count, err = strconv.ParseInt(util.String([]byte(r)), 10, 64); err != nil {
			return nil, err
		}
	}
...
// Only support [show tables] and [show databases]
type Show struct {
	Type string
}
...
func (*Route) IStatement() {}

func (node *Route) Format(buf *TrackedBuffer) {
}
