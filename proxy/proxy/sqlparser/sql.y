...
// SHOW TABLES/DATABASES 只支持这两种. show_td_statement
%token <empty> SHOW
%token <empty> TABLES DATABASES COBAR_CLUSTER
...
%type <statement> show_td_statement
%type <statement> route_statement
...
create_statement:
  ...
  {
    $$ = &DDL{Action: AST_TABLE_CREATE, Table: $4, NewName: $4}
  }
| CREATE constraint_opt INDEX sql_id using_opt ON ID force_eof
  {
    $$ = &DDL{Action: AST_INDEX_CREATE, Table: $7, NewName: $4}
  }
| CREATE VIEW sql_id force_eof
  {
    $$ = &DDL{Action: AST_TABLE_CREATE, NewName: $3}
  }
| CREATE DATABASE not_exists_opt ID force_eof
  {
    $$ = &DDL{Action: AST_DATABASE_CREATE, Database: $4, NewName: $4}
  }

show_td_statement:
  SHOW TABLES force_eof
  {
    $$ = &Show{Type: AST_SHOW_TYPE_TABLES}
  }
| SHOW DATABASES force_eof
  {
    $$ = &Show{Type: AST_SHOW_TYPE_DATABASES}
  }
| SHOW COBAR_CLUSTER force_eof
  {
    $$ = &Show{Type: AST_SHOW_TYPE_COBAR_CLUSTER}
  }
...
drop_statement:
  ...
  {
    $$ = &DDL{Action: AST_TABLE_DROP, Table: $4}
  }
| DROP DATABASE exists_opt ID
  {
    $$ = &DDL{Action: AST_DATABASE_DROP, Database: $4}
  }
